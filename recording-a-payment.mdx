...
If you don't name a specific order, the bot pays down **Steve's oldest open order first**. If the payment is bigger than that order's balance, the leftover automatically rolls onto the next-oldest open order, and so on:

```
💰 Payment recorded — Steve: $29,000
  Applied SKP-20260801-A1B2C3D4: $10,000 → paid
  Applied SKP-20260916-B83484CB: $19,000 → partially paid ($10,750 remaining)
Steve now owes: $10,750
```

This means you never have to remember or type an order number for the common case — "Payment received Steve 29,000" just works even if Steve has multiple invoices outstanding.
...
```
Payment received Steve 29,000 for SKP-20260916-B83484CB
```
...
If the amount is more than everything Steve currently owes, the bot pays off every open order and tells you about the leftover rather than guessing what to do with it:

```
💰 Payment recorded — Steve: $500
  Applied SKP-20260916-B83484CB: $150 → paid
⚠️ $350 could not be applied — no more open orders for Steve, please check.
Steve now owes: $0
```
